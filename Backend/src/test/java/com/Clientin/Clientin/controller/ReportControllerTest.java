package com.Clientin.Clientin.controller;
    ...
    class ReportControllerTest {
        ...
        private final ReportDTO validDto = createValidDto();
        ...
        void getById_ShouldReturnResourceWithLinks() throws Exception {
            given(reportService.findById(TEST_ID)).willReturn(Optional.of(validDto));
        ...
        @Test
        @WithMockUser(authorities = "SCOPE_REPORT_WRITE")
        void create_ShouldReturnCreatedWithLocation() throws Exception {
            given(reportService.create(any())).willReturn(validDto);
        ...
        void getAll_ShouldReturnPagedResources() throws Exception {
            Page<ReportDTO> page = new PageImpl<>(List.of(validDto));
            given(reportService.findAll(any())).willReturn(page);
            ...
            mockMvc.perform(get(BASE_URL)
                    ...
                    .param("sort", "id,desc"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$._embedded.reportList[0].id").exists())
                .andExpect(jsonPath("$._links.self.href").exists());
        }

        @Test
        @WithMockUser(authorities = "SCOPE_REPORT_DELETE")
        void delete_ShouldReturnNoContent() throws Exception {
            given(reportService.exists(TEST_ID)).willReturn(true);
        ...
        @Test
        @WithMockUser(authorities = "SCOPE_REPORT_UPDATE")
        void partialUpdate_ShouldHandleInvalidData() throws Exception {
            given(reportService.partialUpdate(eq(TEST_ID), any()))
        ...
        private ReportDTO createValidDto() {
